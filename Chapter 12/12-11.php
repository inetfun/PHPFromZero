...
<?
    ...
    mysql_query("set names 'gb2312'");
      //生成查询记录数的SQL语句
    $sql = "select count(*) as total from tnews";
        //查询记录数
    $rst = mysql_query($sql);
       //取出一条记录
	...
	if($pageno>0){
     //把$PHP_SELF转换为可以在URL上使用的字符串，这样的话就可以处理中文目录或中文文件名
       ...
       if($pageno>1){//显示上一页的裢接
          echo '<a href="14-11.php' . $href . '?pageno=' . ($pageno-1) . '">上一页</a>  ';
       }
       else{
          echo '上一页  ';
       }
       for($i=1;$i<$pageno;$i++){
          echo '<a href="14-11.php' . $href . '?pageno=' . $i . '">' . $i . '</a>  ';
       }
       echo $pageno . '  ';
       for($i++;$i<=$pagecount;$i++){
          echo '<a href="14-11.php' . $href . '?pageno=' . $i . '">' . $i . '</a>  ';
       ...
       if($pageno<$pagecount){//显示下一页的裢接
          echo '<a href="14-11.php' . $href . '?pageno=' . ($pageno+1) . '">下一页</a>  ';
       }
       else{
          echo '下一页  ';
       }
       ...
       $offset = ($pageno-1) * $pagesize;
         //生成查询本页数据的SQL语句
       $sql = "select * from tnews LIMIT $offset,$pagesize";
         //查询本页数据
       $rst = mysql_query($sql);
           //取得字段总数
